...
function C:onRefresh()
    ...
    if targetId then
        ...
        if not targetVeh.pursuit.roadblockPos or (targetVeh.pursuit.roadblockPos and
            be:getObjectByID(self.veh.id):getPosition():squaredDistance(targetVeh.pursuit.roadblockPos) > 400) then
            -- ignores this if vehicle is at a roadblock
            self:setAction('pursuitStart', {
                targetId = targetId
            })
        end
        local dirBias = gameplay_police.getPoliceVars().spawnDirBias
...
function C:onTrafficTick(dt)
    for id, veh in pairs(gameplay_traffic.getTrafficData()) do -- update data of potential targets
        if id ~= self.veh.id and veh.role.name ~= 'police' and not veh.ignorePolice then
            if not self.validTargets[id] then
                self.validTargets[id] = {}
            end
    ...
    if self.veh.isAi and self.targetId and self.state ~= 'disabled' and self.veh.state == 'active' and self.veh.damage >
        self.veh.damageLimits[3] then
...
function C:onUpdate(dt, dtSim)
    ...
    if self.veh.isAi then
        ...
        if self.flags.pursuit and self.state ~= 'none' and self.state ~= 'disabled' and self.veh.vars.aiMode ==
            'traffic' then
            local minSpeed = (4 - targetVeh.pursuit.mode) * 3
            if self.flags.roadblock == 1 then
                minSpeed = 0
            end

            if (self.flags.roadblock and
                targetVeh.vel:dot((targetVeh.pos - targetVeh.pursuit.roadblockPos):normalized()) >= 9) or
                targetVeh.pursuit.timers.evadeValue >= 0.5 then
                obj:queueLuaCommand('ai.setSpeedMode("off")')
            ...
            if targetVeh.pursuit.mode < 3 and targetVisible and targetVeh.speed <= minSpeed then
                if self.state == 'chase' and distSq <= brakeDistSq and
                    targetVeh.driveVec:dot(targetVeh.pos - self.veh.pos) > 0 then -- pull over near target vehicle
...
return function(...)
    return require('/lua/ge/extensions/gameplay/traffic/baseRole')(C, ...)
end
