...
local function checkRoadblock(vehIds, rbWidth, useLength) -- checks how many of the input vehicles can fit in a roadblock
    ...
    for _, id in ipairs(vehIds) do
        ...
        table.sort(temp, function(a, b)
            return a[2] < b[2]
        end) -- smallest to largest length
    end
...
local function placeRoadblock(vehIds, pos, rot, placeData) -- places a roadblock
    if type(vehIds) ~= 'table' or not pos or not rot then
        return
    end
    ...
    core_multiSpawn.placeGroup(vehIds, {
        ignoreAdjust = true,
        ignoreSafe = true,
        customTransforms = transforms
    }) -- delays some frames to improve performance
end
...
local function setPursuitMode(mode, targetId, policeId) -- sets pursuit mode; -1 = busted, 0 = off, 1 and higher = pursuit level
    ...
    if not traffic[targetId] or targetId == policeId then
        return
    end
    ...
    for id, veh in pairs(policeVehs) do
        if veh.role.state ~= 'disabled' and (not policeId or id == policeId) then
            ...
            elseif mode >= 1 then
                ...
                veh.role:setAction('pursuitStart', {
                    mode = mode,
                    targetId = targetId
                })
                pursuit.score = lastMode <= mode and max(pursuit.score, vars.scoreLevels[mode]) or
                                    min(pursuit.score, vars.scoreLevels[mode])
                pursuit.cooldown = false
            end
        end
    end

    extensions.hook('onPursuitModeUpdate', targetId, {
        mode = mode
    })
end
    ...
    if obj and obj:isPlayerControlled() then
        ...
        if traffic[oldId] and traffic[newId] then
            ...
            if (inVeh or outVeh) and traffic[oldId].pursuit.mode > 0 then
                ...
                traffic[newId].queuedFuncs.pursuitChange = {
                    timer = 0.1,
                    func = gameplay_police.setPursuitMode,
                    args = {mode, newId}
                }

                if inVeh and traffic[newId].pursuit.policeVisible then
                    -- force the target to get arrested
                    traffic[newId].queuedFuncs.autoArrest = {
                        timer = 0.2,
                        func = gameplay_police.arrestVehicle,
                        args = {newId, gameplay_traffic.showMessages}
                    }
                end
            end
        end
...
local function onUpdate(dt, dtSim)
    if not M.enabled or not be:getEnabled() then
        return
    end
    if gameplay_traffic.getState() ~= 'on' or not next(policeVehs) then
        suspectActive = false
    ...
    for id, veh in pairs(gameplay_traffic.getTrafficData()) do
                ...
                if pursuit.mode < i and pursuit.score >= score then
                    if pursuit.mode == 0 then -- activates only nearest police vehicle at start of pursuit
                        if not veh.queuedFuncs.pursuitStart then
                            ...
                            veh.queuedFuncs.pursuitStart = {
                                timer = delay,
                                func = setPursuitMode,
                                args = {i, id, bestPoliceId}
                            } -- small delay for lights & sirens
                        end
                    else
                        setPursuitMode(i, id)
                    end
        ...
        if pursuit.mode ~= 0 then
                ...
                if bestPoliceId and bestDist < square(arrestDist) then
                    ...
                    if pursuit.mode == 3 then
                        ...
                        if vars.roadblockFrequency > 0 and pursuit.timers.roadblock == 0 then
                            ...
                            if not pursuit.roadblockPos or
                                (pursuit.roadblockPos and veh.pos:squaredDistance(pursuit.roadblockPos) > 400) then
                                ...
                                for otherId, otherVeh in pairs(policeVehs) do -- first, check for police vehicles that are out of sight
                                    if be:getObjectByID(otherId):getActive() and otherVeh.role.validTargets[id] and
                                        otherVeh.role.validTargets[id].dist > 10000 and otherVeh.distCam > minDist and
                                        otherVeh.respawn.sightDirValue < 100 then
                            ...
                            if vehIds[min(2, count)] then -- at least 2 vehicles, or 1 if it is the only one
                                spawnData = gameplay_traffic.findSpawnPoint(core_camera.getPosition(),
                                    core_camera.getForward(), 100, 240, {
                                        pathRandom = 0
                                    })
                                if spawnData then
                                    ...
                                    if policePropIds[1] then
                                        ...
                                        for _, v in ipairs(newPropIds) do
                                            maxPropLength = max(maxPropLength,
                                                be:getObjectByID(v).initialNodePosBB:getExtents().y)
                                        end
                                    end
                                    ...
                                    placeRoadblock(newVehIds, spawnData.pos,
                                        quatFromDir(spawnData.dir, spawnData.normal), {
                                            angle = angle,
                                            centerAngle = 0,
                                            width = rbWidth
                                        })
                                    if newPropIds then
                                        placeRoadblock(policePropIds,
                                            spawnData.pos - spawnData.dir * (maxPropLength * 0.5 + 2),
                                            quatFromDir(spawnData.dir, spawnData.normal), {
                                                angle = -90,
                                                width = rbWidth
                                            })
            ...
            if not pursuit.roadblockNear and pursuit.roadblockPos and veh.pos:squaredDistance(pursuit.roadblockPos) <=
                400 then -- increment roadblock counter
        ...
        pursuit.timers.arrestValue = pursuit.mode ~= -1 and
                                         clamp(pursuit.timers.arrest / max(1e-12, vars.arrestLimit), 0, 1) or 1
...
local function onSerialize()
    local data = {
        vars = deepcopy(vars),
        propIds = deepcopy(policePropIds)
    } -- no need to cache police ids, they should automatically get reprocessed
    onTrafficStopped()
    return data
end
