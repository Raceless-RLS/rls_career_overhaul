...
local function getPlPerkValue(policyId, perkName)
    return
        availablePolicies[policyId].perks[perkName].changeability.changeParams.choices[plPoliciesData[policyId].perks[perkName]]
end
...
local function getRateIncrease(vehId, fullcost, paid)
    ...
    return 1 + math.floor((covering /
                              ((700 -
                                  M.calculatePremiumDetails(insuredInvVehs[tostring(vehId)]).perksPriceDetails['repairTime']
                                      .price) * 100)) * 100) / 100
end
...
local function startRepair(inventoryId, repairOptionData, callback)
    ...
    -- the actual repair
    local paintRepair = repairOption.isPolicyRepair and
                            (repairOption.paintRepair or
                                getPlPerkValue(insuredInvVehs[tostring(inventoryId)], "paintRepair") == true)
    local data = {
...
local function getRepairData()
    ...
    data.numberOfBrokenParts = getNumberOfBrokenParts(career_modules_inventory.getVehicles()[vehicleToRepairData.id]
                                                          .partConditions)
    return data
end
...
M.addTicketEvent = addTicketEvent
