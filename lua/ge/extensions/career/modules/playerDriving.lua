...
local function setTrafficVars()
  ...
  gameplay_police.setPursuitVars({ arrestRadius = 15 })
...
local function setupTraffic(forceSetup)
  ...
  if playerIsCop == true then
    if forceSetup or (gameplay_traffic.getState() == "off" and not gameplay_traffic.getTrafficList(true)[1] and playerData.trafficActive == 0) then
      ...
      playerData.trafficActive = M.debugMode and _devTraffic.active or
          amount                       -- store the amount here for future usage
      ...
      gameplay_traffic.setupTraffic(M.debugMode and _devTraffic.traffic + extraAmount or amount + extraAmount, 0,
        { policeAmount = policeAmount, simpleVehs = true, autoLoadFromFile = true, pos = trafficPos, rot = trafficRot })
      setTrafficVars()
...
local function onPursuitAction(vehId, data)
  ...
  if data.type == "start" then -- pursuit started
    ...
    if hasLicensePlate(inventoryId) then
      gameplay_police.setPursuitVars({ evadeLimit = 55 })
    else
      gameplay_police.setPursuitVars({ evadeLimit = 35 })
    end
  ...
  elseif data.type == "evade" then
    if not gameplay_walk.isWalking() then
      ...
      if playerIsCop == true then
        ...
        career_modules_payment.pay({ money = { amount = pity } },
        ...
        if playerIsCop == false then
          ...
          career_modules_payment.pay({ money = { amount = -reward } },
  ...
  elseif data.type == "arrest" then -- pursuit arrest, make the player pay a fine
    if playerIsCop == true then
      ...
      career_saveSystem.saveCurrent()
      career_modules_payment.pay({ money = { amount = -bonus } }, { label = "Arrest Bonus" })
      ...
      if not inventoryId or hasLicensePlate(inventoryId) then
        ...
        career_saveSystem.saveCurrent()
        career_modules_payment.pay({ money = { amount = fine } },
          { label = "Fine for being arrested by the police (No License Plate)" })
        ui_message(
          translateLanguage("ui.traffic.policeFine", "You got fined by the police extra for missing licenseplate: $") ..
          fine, 5, "careerPursuit")
        career_saveSystem.saveCurrent()
      end
    end
  end
...
local function deleteTrailers(veh)
  ...
  if trailerDataThisVeh then
    ...
    career_modules_inventory.removeVehicleObject(career_modules_inventory.getInventoryIdFromVehicleId(trailerDataThisVeh
      .trailerId))
  end
end
...
local teleportTrailerJob = function(job)
  ...
  spawn.safeTeleport(trailer,
    vehBBCenter - vehicle:getDirectionVector() * (vehBB:getHalfExtents().y + trailerBB:getHalfExtents().y), vehRot, nil,
    nil, nil, true, args.resetVeh)
